...
public interface IPublicacionDao extends CrudRepository<Publicacion, Long>{
	...
	@Query("select p from Publicacion p join fetch p.usuario u join fetch p.ubicacion b join fetch p.publicacionCaracteristicas r join fetch r.caracteristica where p.id=?1")
